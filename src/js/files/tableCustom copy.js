...
class RoaswayTable {
  ...
  stretchColumns(state, totalFitWidth, tableWidth) {
    ...
    nonFitColumns.forEach(col => {
        ...
        if (col.index === 3) {
            console.log('Column 3 stretch:', {
                originalWidth: col.width,
                ratio,
                newWidth: state.maxColumnWidths[col.index]
            });
        }
    });
  }
  ...
  getMinContentWidth(header, rows, index) {
    const getCellFullWidth = (cell) => {
        ...
        // Логируем только для 3-й колонки
        if (index === 3) {
    ...
    if (!isFit) {
        ...
        // Логируем только для 3-й колонки
        if (index === 3) {
            console.log('After adding 30px:', maxWidth);
        }
    }
    ...
    // Логируем только для 3-й колонки
    if (index === 3) {
        console.log('Final width:', maxWidth);
    }
  ...
  setCellWidth(cell, width, isFlexible = false) {
    ...
    if (isFlexible) {
      if (cell.classList.contains('fit')) {
        // Для fit колонок фиксированная ширина
  ...
  setupCells(state) {
    ...
    state.rows.forEach((row, rowIndex) => {
      ...
      row.querySelectorAll('.table-line').forEach((cell, cellIndex) => {
        ...
        const width = state.maxColumnWidths[cellIndex];

        if (state.headerCells[cellIndex].classList.contains('fit')) {
          cell.classList.add('fit');
        }
        this.setCellWidth(cell, width, isFlexible);
      });
    });
  }
  ...
  startColumnResize(e, resizable, index, state) {
    ...
    const onMouseMove = (moveEvent) => {
      requestAnimationFrame(() => {
        ...
        if (state.isFlexible) {
          // В гибком режиме:
          // 1. Обновляем только текущую колонку
          const currentHeader = state.headerCells[index];
          this.setCellWidth(currentHeader, newWidth, state.isFlexible);
          
          // 2. Обновляем все ячейки в этой колонке
          state.rows.forEach(row => {
            const cell = row.querySelectorAll('.table-line')[index];
            if (cell) this.setCellWidth(cell, newWidth, state.isFlexible);
          });
          
          // 3. Проверяем и восстанавливаем fit-колонки
          state.headerCells.forEach((header, idx) => {
            if (idx !== index && header.classList.contains('fit')) {
              ...
              state.rows.forEach(row => {
                const fitCell = row.querySelectorAll('.table-line')[idx];
                if (fitCell) this.setCellWidth(fitCell, fitWidth, state.isFlexible);
              });
            }
          });
        } else {
          ...
          state.rows.forEach((row) => {
            const cell = row.querySelectorAll('.table-line')[index];
            this.setCellWidth(cell, newWidth, state.isFlexible);
          });
        }
   ...
   */
  updateShadowDimensions(state, shadowContent) {
    ...
    // Проверяем, должна ли тень быть видна
  ...
  checkShadowVisibility(state, shadowContent) {
    ...
    // Если sticky-элемент касается левого края, показываем тень
    const shouldShowShadow = Math.abs(firstStickyRect.left - wrapperRect.left) <= 1;
  ...
  adjustStickyColumns(state) {
    ...
    if (!stickyHeaders.length || !this.isNonMobileWidth()) return;

    let leftOffset = 0;
    stickyHeaders.forEach((stickyElement) => {
      stickyElement.style.left = `${leftOffset}px`;
      leftOffset += stickyElement.offsetWidth;
    });

    state.rows.forEach((row) => {
      let stickyRowLeftOffset = 0;
      row.querySelectorAll('.sticky-row').forEach((stickyRowElement) => {
        stickyRowElement.style.left = `${stickyRowLeftOffset}px`;
        stickyRowLeftOffset += stickyRowElement.offsetWidth;
      });
    });
